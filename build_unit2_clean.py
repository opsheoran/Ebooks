...
<head>
    ...
    <style>
        ...
        .section-card h3, .section-card h4 {
            font-family: 'Playfair Display', serif; color: var(--navy-light); margin: 28px 0 12px;
        }
...
def extract_and_combine(file2, file3):
    ...
    # Flatten the tree to grab all sections including nested ones properly
    for s in soup2.find_all(['div', 'section'], class_='section'):
        if not s.get('id') or 'exercises' in s.get('id').lower(): continue
        s.extract() # Remove from tree so we flatten it
    ...
    # Re-parse original file for global scripts (chart.js codes) since we mutated soup2
    with open(file2, 'r', encoding='utf-8') as f:
        s2_fresh = BeautifulSoup(f.read(), 'html.parser')
        for s in s2_fresh.find_all('script'):
    ...
    for idx, section in enumerate(all_sections):
        ...
        # Remove all remaining inline scripts from the text bodies to prevent duplicated variables
        for script in section.find_all('script'):
            script.decompose()

        eng_html_raw = "".join([str(child) for child in section.children])

        # Apply math derivation classes using exact stat-102 formatting
        # Wrap block equations (separate lines) in dashed rectangles (formula-box)
        # Using a regex to find equations that are on their own lines or wrapped in <p> tags
        ...
        # Apply derivation styling to inline equations that sit alone in a paragraph
        ...
        # Restore visual blocks in a shared container
