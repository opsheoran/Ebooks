...
def get_verbatim_content():
    # Read the original Chapter1.html
    with open('Stat-101/Chapter1.html', 'r', encoding='utf-8') as f:
        soup = BeautifulSoup(f.read(), 'html.parser')

    # The content is mostly inside elements after "1.1 Introduction"
    # We will find the Introduction heading and gather all content up to the Exercises heading
    start_node = None
    end_node = None
    
    for h2 in soup.find_all('h2'):
        if 'Introduction' in h2.get_text():
            start_node = h2
        if 'Exercises' in h2.get_text():
            end_node = h2
            break
    ...
    while current and current != end_node:
        if current.name == 'h2':
            ...
            text = re.sub(r'^1\.\d+\s*', '', text) # Remove leading numbers like 1.1
            content_html += f'<h2>{text}</h2>\\n'
        elif current.name == 'h3':
            text = current.get_text(strip=True)
            text = re.sub(r'^1\.\d+\.\d+\s*', '', text)
            content_html += f'<h3>{text}</h3>\\n'
        elif current.name == 'h4':
            text = current.get_text(strip=True)
            text = re.sub(r'^1\.\d+\.\d+\.\d+\s*', '', text)
            content_html += f'<h4>{text}</h4>\\n'
        else:
            # Just append the raw HTML of the node (p, div, ul, ol, table)
            if current.name is not None and current.name not in ['header', 'nav']:
                raw_html = str(current)
                # Remove the conversational string if present
                raw_html = raw_html.replace('Hello students, welcome to the BSc First Year Statistics course.', '')
                raw_html = raw_html.replace('Hello students, welcome to the BSc First Year', '')
                raw_html = raw_html.replace('Statistics', 'Statistics')
                raw_html = raw_html.replace('course.', '')
                
                content_html += raw_html + '\\n'
                
        current = current.find_next_sibling()

    # Wrap the extracted content in a single topic section for verbatim display
    wrapped_html = f'''
        <div id="verbatim-theory" class="topic-section active">
            <section class="section-card">
...
<body>
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <ul class="nav-list">
            <li class="nav-item"><a class="nav-link active" href="#" onclick="showTopic('verbatim-theory');return false;"><span class="nav-num">📖</span> Read Chapter</a></li>
        </ul>
...
theory_html = get_verbatim_content()
...
print("Stat-101/Unit-I.html generated successfully with verbatim extraction from Chapter1.html.")
