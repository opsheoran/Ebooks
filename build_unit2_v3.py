...
<head>
    ...
    <style>
        ...
        /* ── MATH & DERIVATION STYLES (Stat-102 matching) ───────────────────────── */
        .formula-box {
            background: linear-gradient(135deg, #EEF2FA 0%, #F7F5FF 100%);
        ...
        .theorem, .definition, .example, .note {
            border-radius: 8px;
            padding: 18px 22px;
            margin: 20px 0;
            position: relative;
        ...
        .theorem { background: var(--sage-light); border-left: 5px solid var(--sage); }
        ...
        .question {
            background: #fff; border: 1px solid var(--border); border-left: 4px solid var(--navy-light);
            padding: 16px 20px; margin: 14px 0 6px; border-radius: 0 8px 8px 0;
            font-size: 1rem; color: var(--text-dark); line-height: 1.7;
        }
        .question strong { color: var(--navy); }
        .mcq-options p { margin: 8px 0 0; color: var(--text-mid); font-size: .95rem; }
        .toggle-btn {
            background: var(--navy); color: #fff; border: none; padding: 7px 18px;
            border-radius: 20px; cursor: pointer; font-family: 'DM Sans', sans-serif;
            font-size: .82rem; font-weight: 500; display: inline-flex; align-items: center;
            gap: 5px; transition: all .25s; margin: 4px 0 8px; letter-spacing: .3px;
        }
        .toggle-btn:hover { background: var(--navy-light); transform: translateY(-2px); box-shadow: 0 4px 12px rgba(27,42,74,.3); }
        .answer {
            display: none; background: #F0FBF5; border-left: 4px solid var(--sage);
            padding: 16px 20px; margin: 2px 0 12px; border-radius: 0 8px 8px 0;
            animation: fadeSlideIn .3s ease; color: var(--text-dark) !important;
        ...
        .section-card h2 {
            ...
            color: var(--navy); margin: 40px 0 20px; padding-bottom: 14px; border-bottom: 2px solid var(--border);
        }
        .section-card h3 {
            font-family: 'Playfair Display', serif; font-size: 1.35rem; font-weight: 700;
            color: var(--navy-light); margin: 28px 0 12px;
        }
        .section-card h4 {
            font-family: 'DM Sans', sans-serif; font-size: 1.05rem; font-weight: 600;
            color: var(--text-dark); margin: 20px 0 8px;
        }
        .section-card p, .section-card ul, .section-card ol {
            font-size: 1.05rem; line-height: 1.8; color: var(--text-mid); margin-bottom: 16px;
        }
        
        table { width: 100%; border-collapse: collapse; margin: 20px 0; border-radius: 8px; overflow: hidden; box-shadow: var(--shadow-sm); }
        table th, table td { border: 1px solid var(--border); padding: 10px; text-align: left; }
        table th { background: var(--navy); color: white; font-family: 'DM Sans', sans-serif; font-weight: 600; }
...
<body>
    ...
    <script src="chart-tool-script.js"></script>
...
def extract_and_combine(file2, file3):
    ...
    for section in all_sections:
        ...
        # Determine Title and fix hierarchy across the two files
        ...
        # Extract visual blocks to prevent duplication and hiding issues
        visual_blocks = []
        for visual in section.find_all(['canvas', 'div'], class_=lambda c: c in ['chart-container', 'chart-item', 'cg-mb-6'] if c else False):
            if visual.name == 'canvas':
                if visual.parent and visual.parent.get('class') and any(c in visual.parent.get('class') for c in ['chart-container', 'chart-item']):
                    continue
            placeholder = f"[[VISUAL_BLOCK_{len(visual_blocks)}]]"
            visual_html = str(visual)
            visual_blocks.append((placeholder, visual_html))
            visual.replace_with(placeholder)

        # Apply derivation styling to mathematical text blocks matching Stat-102
        for p in section.find_all('p'):
            text = p.get_text(strip=True)
            if '\\[' in text and '\\]' in text:
                # Block equation -> formula-box
                p.name = 'div'
                p['class'] = p.get('class', []) + ['formula-box']
            elif text.startswith('\\(') and text.endswith('\\)'):
                # Inline equation forming the whole paragraph -> derivation
                p['class'] = p.get('class', []) + ['derivation']
        ...
        eng_parts = re.split(r'(\[\[VISUAL_BLOCK_\d+\]\])', eng_html_raw)
        hin_parts = re.split(r'(\[\[VISUAL_BLOCK_\d+\]\])', hin_html_raw)

        # Reconstruct interleaving text and visuals
        section_content = ""
        for i in range(len(eng_parts)):
            part_eng = eng_parts[i]
            part_hin = hin_parts[i]
            if part_eng.startswith('[[VISUAL_BLOCK_'):
                # Look up the actual visual HTML
                vis_html = next(html for p, html in visual_blocks if p == part_eng)
    ...
    return nav_links, sections_html

print("Processing Unit II...")
nav_links_html, sections_html = extract_and_combine('Stat-101/Chapter2.html', 'Stat-101/Chapter3.html')

print("Generating True Hinglish exercises...")
exercises_html = generate_unit2_exercises()

print("Replacing placeholders...")
final_html = html_template.replace('{NAV_LINKS_PLACEHOLDER}', nav_links_html)\
...
print("Stat-101/Unit-II.html fully generated with shared visuals, derivations, and continuous numbering!")
