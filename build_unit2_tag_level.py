...
def extract_and_combine(file2, file3):
    ...
    for idx, section in enumerate(all_sections):
        ...
        # To prevent breaking DOM layout, we do tag-level duplication for text elements
        tags_to_translate = ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'table', 'ul', 'ol', 'blockquote']
        for tag in section.find_all(tags_to_translate):
            # Skip if inside another translating tag
            if any(parent.name in tags_to_translate for parent in tag.parents):
                continue
            
            # Skip formulas
            if tag.attrs and ('formula-box' in tag.attrs.get('class', []) or 'derivation' in tag.attrs.get('class', [])):
                # We leave formulas completely untouched, they will show in both languages naturally!
                continue

            # Skip tags that contain canvas (e.g. if someone accidentally put a canvas in a p)
            if tag.find('canvas'):
                continue

            eng_tag = BeautifulSoup(str(tag), 'html.parser').contents[0]
            if not hasattr(eng_tag, 'get'):
                continue
            eng_tag['class'] = eng_tag.get('class', []) + ['english-content']
            
            hin_tag_str = translate_html_block(str(tag))
            hin_tag = BeautifulSoup(hin_tag_str, 'html.parser').contents[0]
            if not hasattr(hin_tag, 'get'):
                continue
        ...
        sections_html += f'''
            <div class="topic-section" id="{sec_id}">
                <section class="section-card">
                    {"".join([str(child) for child in section.children])}
                </section>
            </div>
        '''
...
print("Stat-101/Unit-II.html fully generated using tag-level isolation!")
