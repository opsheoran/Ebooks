...
def extract_and_combine(file2, file3):
    ...
    # Extract scripts that should run globally (chart.js codes) BEFORE flattening
    shared_scripts = ""
    for s in soup2.find_all('script'):
        if not s.get('src') and not 'MathJax' in s.text:
            shared_scripts += str(s) + '\n'
            s.decompose() # Remove from soup so it doesn't get processed in sections
    ...
    for idx, section in enumerate(all_sections):
        ...
        # And DO NOT duplicate canvases or their containers
        visual_blocks = []
        for visual in section.find_all(['canvas', 'div'], class_=lambda c: c in ['chart-container', 'chart-item', 'cg-mb-6'] if c else False):
            ...
            placeholder = eng_soup.new_tag("div", id=placeholder_id, **{'class': 'visual-placeholder'}) if 'eng_soup' in locals() else BeautifulSoup(f'<div id="{placeholder_id}" class="visual-placeholder"></div>', 'html.parser').div
            
            visual_html = str(visual)
            visual_blocks.append((placeholder_id, visual_html))
            visual.replace_with(placeholder)

        # Remove any stray inline scripts to completely prevent duplicated JS variable declarations
        ...
        # Build final section content securely
        ...
        sections_html += f'''            <div class="topic-section" id="{sec_id}">
